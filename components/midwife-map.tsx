...
export function MidwifeMap({ midwives, onMarkerClick, className = "" }: MidwifeMapProps) {
  ...
  const getMapUrl = () => {
    ...
    const apiKey = process.env.NEXT_PUBLIC_MAPTILER_KEY || process.env.MAPTILER_KEY // Fallback for direct MAPTILER_KEY if not NEXT_PUBLIC_

    if (!apiKey) {
      console.error(
        "MapTiler API key is not set. Please set NEXT_PUBLIC_MAPTILER_KEY or MAPTILER_KEY environment variable.",
      )
      return "/map-error.png" // Return a placeholder or error image
    }
  ...
  return (
    <Card className={`overflow-hidden relative ${className}`}>
      <div className="relative w-full h-full min-h-[400px]">
        {/* Map Image */}
        <img
          src={getMapUrl() || "/placeholder.svg"} // getMapUrl now handles the API key
        ...
        <div className="absolute top-4 right-4 flex flex-col gap-2">
          <Button variant="outline" size="icon" className="bg-white/90 backdrop-blur-sm" onClick={handleZoomIn}>
            <ZoomIn className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="icon" className="bg-white/90 backdrop-blur-sm" onClick={handleZoomOut}>
            <ZoomOut className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="icon" className="bg-white/90 backdrop-blur-sm" onClick={handleReset}>
